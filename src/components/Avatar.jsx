...
const EYE_SOCKET_TOP_PERCENT = 0.3; // 30% from head top
const EYE_SOCKET_LEFT_PERCENT = 0.2; // 20% from head left for left eye
const EYE_SOCKET_RIGHT_PERCENT = 0.2; // 20% from head right for right eye
...
const Avatar = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (event) => {
      ...
      const headActualLeftInContainer =
        (styles.container.width - HEAD_WIDTH) / 2;
      ...
      const distanceLeft = Math.sqrt(
        deltaXLeft * deltaXLeft + deltaYLeft * deltaYLeft
      );
      const pupilXLeft =
        distanceLeft > 0
          ? (deltaXLeft / distanceLeft) *
            Math.min(distanceLeft, MAX_PUPIL_OFFSET_X)
          : 0;
      const pupilYLeft =
        distanceLeft > 0
          ? (deltaYLeft / distanceLeft) *
            Math.min(distanceLeft, MAX_PUPIL_OFFSET_Y)
          : 0;
      setLeftPupilTransform(
        `translate(calc(-50% + ${pupilXLeft}px), calc(-50% + ${pupilYLeft}px))`
      );
      ...
      const distanceRight = Math.sqrt(
        deltaXRight * deltaXRight + deltaYRight * deltaYRight
      );
      const pupilXRight =
        distanceRight > 0
          ? (deltaXRight / distanceRight) *
            Math.min(distanceRight, MAX_PUPIL_OFFSET_X)
          : 0;
      const pupilYRight =
        distanceRight > 0
          ? (deltaYRight / distanceRight) *
            Math.min(distanceRight, MAX_PUPIL_OFFSET_Y)
          : 0;
      setRightPupilTransform(
        `translate(calc(-50% + ${pupilXRight}px), calc(-50% + ${pupilYRight}px))`
      );
    };
    ...
    <div ref={avatarRef} style={styles.container}>
      ...
      <div style={styles.head}>
        ...
        <div style={styles.rightEyeSocket}>
          <div
            style={{ ...styles.pupil, transform: rightPupilTransform }}
          ></div>
        </div>
      </div>
